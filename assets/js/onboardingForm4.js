...
$( "#shares-desired" ).blur(function() {
  ...
  if (!$.isNumeric(id) || Math.floor(id) != id) {
      ...
      $( "#shares-desired" ).val(Math.ceil(id));
      $("#shares-cost").val(Math.ceil(id) * 13.5);
      if ($( "#shares-desired" ).val() < 80) {
        $( "#shares-desired" ).val(Math.ceil(80));
        $("#shares-cost").val(Math.ceil(80) * 13.5);
      }
  }
});
